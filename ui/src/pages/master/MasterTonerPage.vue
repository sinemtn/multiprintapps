...
const page = useEntityCrudPage({
    crudKey: 'toner',
    title: 'Master Toner',
    subtitle: 'Kelola master toner untuk stok toner dan printer.',
    writeRoles: ['admin'],
    emptyForm: { code: '', name: '', category: '', status: 'active' },
    columns: [
        { key: 'code', label: 'Code' },
        { key: 'name', label: 'Nama' },
...
<template>
  <PageHeader title="Master Toner" subtitle="Kelola master toner untuk stok toner dan printer.">
    <template #actions>
  ...
  <section>
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <input v-model="page.search.value" class="field-control max-w-sm" placeholder="Cari master toner..." />
  ...
  <DialogModal :open="page.modalOpen.value" :title="page.modalTitle.value" description="Isi data master toner." :submit-label="page.submitLabel.value" :submitting="page.isSubmitting.value" @close="page.closeModal" @submit="page.saveRecord">
    ...
    <FormField id="toner-code" label="Code"><input id="toner-code" :value="page.form.code" class="field-control" placeholder="Code" :disabled="page.isEditing.value" required @input="page.setField('code', $event)" /></FormField>
    <FormField id="toner-name" label="Name"><input id="toner-name" :value="page.form.name" class="field-control" placeholder="Name" required @input="page.setField('name', $event)" /></FormField>
    <FormField id="toner-category" label="Category"><input id="toner-category" :value="page.form.category" class="field-control" placeholder="Category" required @input="page.setField('category', $event)" /></FormField>
    <SelectField id="toner-status" v-model="page.form.status" label="Status" :options="statusOptions" />
  </DialogModal>
</template>
