...
const page = useEntityCrudPage({
    crudKey: 'stock-toner',
    title: 'Stok Toner',
    subtitle: 'Kelola stok toner, lokasi, customer, dan kuantitas.',
    writeRoles: ['admin', 'warehouse'],
    emptyForm: { code: '', location: '', branch: '', qty: 0, customer: '', note: '' },
    columns: [
        { key: 'code', label: 'Toner' },
...
<template>
  <PageHeader title="Stok Toner" subtitle="Kelola stok toner, lokasi, customer, dan kuantitas.">
  ...
  <section>
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <input v-model="page.search.value" class="field-control max-w-sm" placeholder="Cari stok toner..." />
  ...
  <DialogModal :open="page.modalOpen.value" :title="page.modalTitle.value" description="Isi data stok toner." :submit-label="page.submitLabel.value" :submitting="page.isSubmitting.value" @close="page.closeModal" @submit="page.saveRecord">
    ...
    <FormField id="stock-toner-code" label="Toner"><input id="stock-toner-code" :value="page.form.code" class="field-control" placeholder="Toner" required @input="page.setField('code', $event)" /></FormField>
    <FormField id="stock-toner-location" label="Lokasi"><input id="stock-toner-location" :value="page.form.location" class="field-control" placeholder="Lokasi" @input="page.setField('location', $event)" /></FormField>
    <SelectField v-if="branchRequired" id="stock-toner-branch" v-model="page.form.branch" label="Cabang" :options="branchOptions" />
    <FormField v-else id="stock-toner-branch" label="Cabang"><input id="stock-toner-branch" :value="page.form.branch" class="field-control" placeholder="Cabang" @input="page.setField('branch', $event)" /></FormField>
    <FormField id="stock-toner-qty" label="Kuantitas"><input id="stock-toner-qty" :value="page.form.qty" class="field-control" type="number" min="0" placeholder="Kuantitas" required @input="page.setNumberField('qty', $event)" /></FormField>
    <SelectField id="stock-toner-customer" :model-value="page.form.customer" label="Customer" :options="customerOptions" @update:model-value="setCustomer" />
    <FormField id="stock-toner-note" label="Catatan"><textarea id="stock-toner-note" :value="page.form.note" class="field-control min-h-24" placeholder="Catatan" @input="page.setField('note', $event)" /></FormField>
  </DialogModal>
</template>
